...
export function InvoiceRecordWorkspace({ projectId, invoiceId }: { projectId: string; invoiceId: string }) {
  ...
  const pay = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!detail || Number(amount) <= 0) return; setWorking(true); try { await recordProjectPayment(projectId, invoiceId, { amount: Number(amount), paidDate: new Date().toISOString().slice(0, 10), reference: reference.trim() || undefined }); setAmount(''); setReference(''); await refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : 'Payment could not be recorded.'); } finally { setWorking(false); } };
