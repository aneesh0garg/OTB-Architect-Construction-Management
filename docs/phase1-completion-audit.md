...
| Documents and drawings                                 | Direct S3/MinIO upload, batch preparation, SHA-256 integrity verification, revision supersession, site-context metadata, signed download, 2D viewer, and access-checked, auditable comments/pins. Dedicated controlled-record routes keep review, viewer, transmittal, and discussion work out of the register. |
| Field and execution                                    | Durable mobile observation capture/sync, structured offline observations, local photo capture, discussion sync, site visits, selected-observation report drafts, review → issue → acknowledgment controls, RFIs, submittals, instructions, meetings, decisions, and state transitions. Observation details, evidence, and discussion open in a dedicated record workspace. |
| Commercial and owner cost                              | Fee/time ledger, GST invoices, payments, owner budgets, commitments, change events, forecast, and CSV exports. Invoice record workspaces expose billing lines, receivables, payment references, and balance reconciliation. |
| Communications and notifications                       | Manual project filing, native notification feed, preferences, quiet-hour deferral, Gmail OAuth/file/send boundary, and explicit WhatsApp Business Phase 2 boundary. Filed messages retain conversation-thread identity and open as conversation records. |
...
`scripts/phase1-smoke.mjs` resets its notification-preference fixtures before each
run, then creates a tenant-scoped project through the pipeline
